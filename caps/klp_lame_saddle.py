...
from build123d import Box, Part, Pos, Rot, Plane, Axis, Sketch, Polyline, Bezier, Curve
...
class CapKind(Enum):
    ...
    INDEX_FINGER_BIG_FLAT = 4  # new outside caps for index finger (double deep, and rotated stem)
    INDEX_FINGER_STD_CUT = 5  # like INDEX_FINGER_STD, but cut off one side, to reduce the distance between the caps
...
class LameSaddleKeyCapCreator:
    ...
    def __init__(self, cap_kind: CapKind = CapKind.ORIG):
        ...
        self._extra_height = self._calc_extra_height(cap_kind)
        self._y_factor = 2.0 if self._cap_kind == CapKind.INDEX_FINGER_BIG_FLAT else 1.0

    @staticmethod
    def _calc_extra_height(cap_kind: CapKind) -> float:
        if cap_kind == CapKind.INDEX_FINGER_BIG_FLAT:
            return 0.6  # 0.2 => small hole, 0.3 => no hole
        elif cap_kind in [CapKind.INDEX_FINGER_STD, CapKind.INDEX_FINGER_STD_CUT]:
            return 0.5  # 0.1 => small hole, 0.2 => no hole
        elif cap_kind == CapKind.INDEX_FINGER_CONCAVE:
            return 1.0  # 0.6 => small hole, 0.7 => no hole
        else:
            return 0.0

    def create(self, with_stems=True) -> Part:
        ...
        if with_stems:
            ...
            cap = cap_without_stems

        #cap -= Pos(X=-10, Y=-5) * Box(20, 20, 20)

        if self._cap_kind == CapKind.INDEX_FINGER_STD_CUT:
            cap -= Pos(X=-17.3) * Box(20, 20, 20)
    ...
    def _iter_stems(self) -> Iterator[Part]:
        ...
        rot = Rot(Z=90) if self._cap_kind == CapKind.INDEX_FINGER_BIG_FLAT else Rot(Z=0)
...
class CapBodyCreator:
    ...
    def __init__(self, y_factor: float = 1.0, extra_height: float = 0.0, cap_kind: CapKind = CapKind.ORIG):
        ...
        self._extra_height = extra_height  # necessary for new cap variants to avoid holes
        self._cap_kind = cap_kind
